...
def describe_image(img_path: str) -> list:
    # API configurations
    asticaAPI_key = "96CF3357-22A1-4897-BB5F-9C6ADF7DB8113954C209-B908-489D-A6D5-4C2B0242BD7A"  # visit https://astica.ai
    asticaAPI_timeout = (
        25  # in seconds. "gpt" or "gpt_detailed" require increased timeouts
    )
    ...
    #         "image.jpg"
    #     )  # use base64 image input (slower)

    asticaAPI_input = get_image_base64_encoding(img_path)

    # vision parameters:  https://astica.ai/vision/documentation/#parameters
    asticaAPI_visionParams = (
        "gpt,describe,objects,faces"  # comma separated, defaults to "all".
    )
    ...
    asticaAPI_result = asticaAPI(
        asticaAPI_endpoint, asticaAPI_payload, asticaAPI_timeout
    )
    ...
    if "status" in asticaAPI_result:
        ...
        if asticaAPI_result["status"] == "success":
            if (
                "caption_GPTS" in asticaAPI_result
                and asticaAPI_result["caption_GPTS"] != ""
            ):
                # print("=================")
                # print("GPT Caption:", asticaAPI_result["caption_GPTS"])
                result_list.append(asticaAPI_result["caption_GPTS"])
            if (
                "caption" in asticaAPI_result
                and asticaAPI_result["caption"]["text"] != ""
            ):
                # print("=================")
                # print("Caption:", asticaAPI_result["caption"]["text"])
                result_list.append(asticaAPI_result["caption"]["text"])
            if (
                "CaptionDetailed" in asticaAPI_result
                and asticaAPI_result["CaptionDetailed"]["text"] != ""
            ):
                # print("=================")
                # print("CaptionDetailed:", asticaAPI_result["CaptionDetailed"]["text"])
                result_list.append(asticaAPI_result["CaptionDetailed"]["text"])
...
description = describe_image(img_path="/home/kwabena/Pictures/macaws.jpg")
print(description[0])
